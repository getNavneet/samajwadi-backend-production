...
export const generateMemberCard = async ({ photoPath, templatePath, name, state, city, phone,cardNo }, outputDir) => {
    ...
    try {
        ...
        ctx.fillStyle = '#000';
        ctx.fillText(`Name: ${name}`, 17, 270);
        ctx.fillText(`Address: ${city}, ${state}`, 17, 320);
        if(phone){
            ctx.fillText(`Contact: ${phone}`, 17, 370);
            ctx.fillText(`Card No: ${cardNo}`, 17, 420);
        }
        else {
            ctx.fillText(`Card No: ${cardNo}`, 17, 370);
        }
